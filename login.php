...
if(isset($_POST['userEmail']) && isset($_POST['userPassword']))
{
  ...
  // echo "<script type='text/javascript'>alert('$msg3');</script>";
	...
	if(!empty($email) && !empty($password))
	{
		...
		$query = "SELECT id FROM user WHERE email='".mysqli_real_escape_string($mysql_connect, $email)."' AND password='".mysqli_real_escape_string($mysql_connect, $password_hash)."'";
        ...
        $query1 = "SELECT id FROM user WHERE email = '$email'";
                                ...
                                $userID = $row['id'];
				header('Location: register_complaint.php?userID='.$userID);
			}
		}
	}
	else
...
<html lang="en">
  ...
  <a href="register_user.php" class="btn btn-secondary btn-primary active" role="button" aria-pressed="true">Sign Up</a>
  </center>
</form>
